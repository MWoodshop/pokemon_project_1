...
RSpec.describe Pokemon, type: :model do
  ...
  # User Story 15
  describe 'scopes' do
    it 'only includes records where the boolean column is true' do
      true_pokemon = FactoryBot.create(:pokemon, name: 'Charmander', pokemon_type: 'Fire', can_evolve: true)
      false_pokemon = FactoryBot.create(:pokemon, name: 'Charizard', pokemon_type: 'Fire', can_evolve: false)

      expect(Pokemon.true_pokemon).to include(true_pokemon)
      expect(Pokemon.false_pokemon).to include(false_pokemon)
    end
  end
  ...
  feature 'Pokemon Index Page', type: :feature do
    before(:each) do
      ...
      @pokemon3 = Pokemon.create(name: 'Charmander', pokemon_type: 'Fire', height: 1, weight: 13, can_evolve: false,
